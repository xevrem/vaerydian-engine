...
export default function ecsRig(callback: EcsRigCallback): void {
  const flagTemp = process.env.REACT_APP_ENABLE_ECS_ARCH;
  process.env.REACT_APP_ENABLE_ECS_ARCH = 'true';
  ...
  const rig: EcsRig = {
    ...
    makeSystemType: ({ needed = [], optional = [], unwanted = [] } = {}): new (
      ...
      class Sys extends EntitySystem {
        ...
        process(entity: Entity, query: Query, _delta: number): void {}
      }
      return Sys;
    },
  };
  ...
  const destroy = () => {
    rig.ecs.cleanUp();
    process.env.REACT_APP_ENABLE_ECS_ARCH = flagTemp;
  };
